...
app.controller('getEmpDetails', function($scope, $stateParams, $http,
	...
	empService.findEmployeeByName($stateParams.email, url, function(r) {
	...
	orgChart.getListOfEmp(url, function(r) {
		...
		for (i = 0; i < $scope.empOrgChart[1].length; i++) {
			$scope.count = $scope.count
					+ $scope.empOrgChart[1][i].empProjectList.length;
		}
...
app
		.controller(
				'empProject',
				function($scope, $stateParams, $http, orgChart, $state) {
					$scope.name = $stateParams.name;
					$scope.id = $stateParams.empId;
					$scope.project = $stateParams.project;
					$scope.email = $stateParams.email;
					...
					$scope.count1 = 0;
					$scope.count2 = 0;
					var url = '/accionlabs/emp/details/' + $stateParams.empId
							+ '/project';
					orgChart
							.getListOfEmp(
									url,
									function(r) {
										$scope.empOrgChart = r;
										if ($scope.empOrgChart.length != 2) {
											$scope.countHigherLevel = $scope.empOrgChart.listOfProjectEmp.length;
											for (i = 0; i < $scope.countHigherLevel; i++) {
												$scope.countLowerlevel = $scope.countLowerlevel
														+ $scope.empOrgChart.listOfProjectEmp[i].listOfProjectEmp.length;
											}
											$scope.count = $scope.countHigherLevel
													+ $scope.countLowerlevel;
										}
										if ($scope.empOrgChart.length === 2) {
											...
											for (i = 0; i < $scope.empOrgChart[0].listOfProjectEmp.length; i++) {
												$scope.countLowerlevel1 = $scope.countLowerlevel1
														+ $scope.empOrgChart[0].listOfProjectEmp[i].listOfProjectEmp.length;
											}
											$scope.count1 = $scope.countHigherLevel1
													+ $scope.countLowerlevel1;
											$scope.count1 = $scope.countHigherLevel1
													+ $scope.countLowerlevel1;
											for (i = 0; i < $scope.empOrgChart[1].listOfProjectEmp.length; i++) {
												$scope.countLowerlevel2 = $scope.countLowerlevel2
														+ $scope.empOrgChart[1].listOfProjectEmp[i].listOfProjectEmp.length;

											}
											$scope.count2 = $scope.countHigherLevel2
													+ $scope.countLowerlevel2;
...
app.controller('empReport', function($scope, $stateParams, $http, orgChart,
	...
	orgChart.getListOfEmp(url, function(r) {
		...
		for (i = 0; i < $scope.empOrgChart[0].length; i++) {
			$scope.count = $scope.count
					+ $scope.empOrgChart[0][i].empProjectList.length;
		}
	});
});

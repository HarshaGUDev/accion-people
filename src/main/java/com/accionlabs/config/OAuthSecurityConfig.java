...
public class OAuthSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	private OAuth2ClientAuthenticationProcessingFilter filter() {
		...
		oAuth2Filter.setAuthenticationSuccessHandler(new SimpleUrlAuthenticationSuccessHandler() {
		    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
		        this.setDefaultTargetUrl("/people");
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http    
				...
				.antMatchers("/people","/js/**","/public/**", "/**.html", "/app/**.js").permitAll()
				...
				// path
				.logoutSuccessUrl("/people").clearAuthentication(true).deleteCookies("JSESSIONID").invalidateHttpSession(true)//
				.permitAll()//
				.and()//

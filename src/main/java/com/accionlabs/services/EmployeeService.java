...
public class EmployeeService {
	...
	public EmployeeBasicDetails empBasicDetails(String email_Id) {
		EmployeeDetails empDetails = employeeRepository.getDetails(email_Id);
	...
	public List<EmpProjectDetails> getProjectWiseDetails(String empid) {
		...
		for (String projectByDetails : listOfProject) {
			...
			List<MinDetails> lowerEmpDetialsList = employeeRepository.getOrgListLower1(empid, projectByDetails);
			for (MinDetails lowerEmpDetials : lowerEmpDetialsList) {
				// EmployeeBasicDetails
				// employeeBasicDetails=filterEmpDetails(lowerEmpDetials);
				listOfDetailsProjectWise.add(lowerEmpDetials);
		...
		Collections.sort(listOfDetails, new EmpManagerWiseComparator());
